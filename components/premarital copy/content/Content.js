...
const Content = ({ data, pb }) => {
  return (
    <section className={`w-full pt-[40px] md:pt-[70px] pb-[30px] md:pb-[40px] bg-[#fff] relativ e ${pb ? "pb-[300px]" : ""}`}>
          ...
          {data.sessions.map((value) => {
            return (
              <div key={value.text} className="w-full lg:w-[48%] h-[auto] sc1350:h-[431px] bg-[#0F2E3C] border-[#707070] border-[1px] rounded-[15px] md:rounded-[30px] px-[20px] md:px-[72px] py-[20px] md:py-[54px]">
                ...
                <p className="fgr text-[12px] md:text-[16px] leading-[16px] md:leading-[25px] text-[#fff] mt-[2%] md:mt-[5%]">{value.text}</p>
              </div>
            )
          })}
        </div>
